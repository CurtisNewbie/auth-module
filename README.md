...
boolean | authmodule.is-app-authorization-checked | whether we should check user's authorization to use current application (`spring.application.name` is used to identify which application is used). (see **auth-service** for more information) | false
